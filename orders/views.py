...
def solo_order(request, pk):
    ...
    try:
        ...
        if user.is_staff or order.user == user: #solo esl usuario administrado o el usuario dueño de la orden puede acceder
            serializer = OrderSerializer(order, many=False)
            return Response(serializer.data)
        else:
        ...
        return Response({'detail': 'Order does not exist'}, status=status.HTTP_400_BAD_REQUEST) #en caso que no exista la orden
